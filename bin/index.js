...
async function GenerateProject(targetDir) {
...
function DownTemplate(projectDir) {
  const remote = 'https://github.com/lin2006yuo/multi-spa-webpack-cli.git';
  const { template, server } = answersConfig;
  console.log({template, server})
  const serveDir = server ? 'server/**' : '!server/**';
  console.log('serveDir', serveDir, projectDir);
  ...
  .action(async (appName) => {
    try {
      ...
      if (fs.pathExistsSync(targetDir)) {
        if (program.force) {
          GenerateProject(appName) // 创建项目；
        }
        ora(
          chalk.red(`！当前目录下，${appName}已存在，请修改名称后重试`)
        ).fail()
        process.exit(1)
      }
      GenerateProject(appName) // 创建项目；
